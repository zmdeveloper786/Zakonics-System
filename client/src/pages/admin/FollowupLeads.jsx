...
const FollowupLeads = () => {
  ...
  const handleConvert = async (e) => {
    ...
    try {
      await axios.post('https://app.zumarlawfirm.com:5000/convertedService', formData, {
  ...
  const fetchLeads = async () => {
    try {
      const res = await axios.get('https://app.zumarlawfirm.com:5000/leads');
      setLeads(res.data);
    } catch (err) {
      setLeads([]);
    }
  ...
  // (Removed duplicate selectedRows, setSelectedRows, allRowIds, isAllSelected, handleSelectAll, handleSelectRow)

  const handleStatusChange = async (leadId, value) => {
    try {
      await axios.put(`https://app.zumarlawfirm.com:5000/leads/${leadId}/status`, { status: value });
    } catch (err) {}
    setLeads(prev => {
      ...
      // Only keep leads with status 'Followup' or 'Follow-up' in the current page
      updated = updated.filter(lead => lead.status === 'Followup' || lead.status === 'Follow-up');
      return updated;
    });
  };
  ...
  const handleEditSave = async () => {
    try {
      await axios.put(`https://app.zumarlawfirm.com:5000/leads/${editModal.lead._id}`, editModal.lead);
  ...
  const handleDeleteLead = async (leadId) => {
    ...
    try {
      await axios.delete(`https://app.zumarlawfirm.com:5000/leads/${leadId}`);
  ...
  // Only show leads with status 'Followup' or 'Follow-up'
  const followupLeads = leads.filter(lead => lead.status === 'Followup' || lead.status === 'Follow-up');
  const allRowIds = followupLeads.map(lead => `${lead._id}`);
  ...
  return (
    <div className="">
      ...
      <div className="text-sm text-gray-500 mb-2">
        ...
        <span className="text-gray-800 font-medium">Follow-up Leads</span>
      </div>

      {/* Header & Buttons */}
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold">Followup Leads</h2>
        ...
        onClick={() => {
          const lead = followupLeads.find(l => l._id === selectedRows[0]);
        ...
        <table className="min-w-full bg-white">
          ...
          <tbody className="text-sm text-gray-700">
            {followupLeads.length > 0 ? (
              ...
              <tr>
                <td colSpan={8} className="p-4 text-center text-gray-500">No contacted leads found.</td>
              </tr>
            )}
          </tbody>
        </table>
