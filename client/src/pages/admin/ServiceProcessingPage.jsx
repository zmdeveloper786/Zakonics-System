...
const ServiceProcessingPage = () => {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        const res = await axios.get('http://app.zumarlawfirm.com:5000/admin/roles', { headers: getAuthHeaders() });
  ...
  const fetchServices = async () => {
    setLoading(true);
    try {
      const res = await axios.get('http://app.zumarlawfirm.com:5000/admin/services', { headers: getAuthHeaders() });
  ...
  const handleAssignEmployee = async (row, employeeName) => {
    try {
      await axios.patch(
        `http://app.zumarlawfirm.com:5000/admin/services/${row._id}/assign`,
    ...
    try {
      ...
      try {
        res = await axios.patch(
          `http://app.zumarlawfirm.com:5000/admin/services/${row._id}/status`,
          { status: nextStatus },
          { headers: getAuthHeaders() }
        );
      } catch (err) {
        if (err.response && err.response.status === 404) {
          res = await axios.patch(
            `http://app.zumarlawfirm.com:5000/admin/services/status/${row._id}`,
            { status: nextStatus },
            { headers: getAuthHeaders() }
          );
        } else {
  ...
  const handlePaymentStatusCycle = async (row) => {
    ...
    try {
      await axios.patch(
        `http://app.zumarlawfirm.com:5000/admin/services/${row._id}/payment-status`,
  ...
  const handleCertificateUpload = async (e) => {
    ...
    try {
      await axios.post(
        `http://app.zumarlawfirm.com:5000/admin/services/${selectedRow._id}/certificate?pending=true`,
        formData,
        {
          headers: {
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      await axios.post('http://app.zumarlawfirm.com:5000/invoices/delete-multiple', { ids: selectedRows }, { headers: getAuthHeaders() });
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        await axios.post(`http://app.zumarlawfirm.com:5000/admin/services/${selectedRow._id}/send-invoice`);
